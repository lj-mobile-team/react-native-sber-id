...
///
+ (void)authWithSberId:(SBKAuthRequest * _Nonnull)request;
/// Запуск авторизации пользователя по Сбер ID, используя единое веб окно авторизации.
/// \param sberIdRequest модель запроса
///
/// \param request модель запроса
///
/// \param svcRedirectUrlString URL для возврата из СБОЛа в МП партнера на откртый SVC
/// со страницей единого портала авторизации
///
///
/// returns:
/// статус успеха запуска авторизации
+ (BOOL)soleLoginWebPageAuthWithSberIdRequest:(SBKAuthRequest * _Nonnull)request svcRedirectUrlString:(NSString * _Nonnull)svcRedirectUrlString SWIFT_WARN_UNUSED_RESULT;
...
///
- (nonnull instancetype)initWithType:(enum LoginButtonStyle)type textType:(enum LoginButtonTextType)textType desiredHeight:(CGFloat)desiredHeight desiredWidth:(CGFloat)desiredWidth;
